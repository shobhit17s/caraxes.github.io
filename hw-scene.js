...
(function (HW) {
  ...
  HW.createScene = function (canvas, worlds) {
    ...
    function drawSpotLabel(ctx, spot) {
      if (!spot || spot.kind !== 'link' || !spot.label) return;
      ctx.save();
    ...
    function frame() {
      ...
      for (var i = 0; i < planets.length; i++) planets[i].update(cam, time, motion);
      ...
      canvas.style.cursor = (hoverSpot || overPlanet) ? 'pointer' : 'default';
    ...
    var scene = {
      ...
      begin: function () {
        ...
        canvas.addEventListener('click', function (e) {
          ...
          if (spot && spot.kind === 'link') { openDoor(spot.href); return; }
