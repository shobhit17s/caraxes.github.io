...
(function (CS) {
  ...
  CS.initWireframes = function () {
    var slide = document.querySelector('.t-wireframe');
    if (!slide) return;
    var flows = data().workflows || [];
    ...
    var action = data().workflowAction;
    if (right && action) {
    ...
    CS.makeTabs(panel, flows, function (w) { flow = w; index = 0; show(); });
  ...
  /* ---------- the backend slide: toggles pick a diagram ---------- */
  CS.initDiagrams = function () {
    var slide = document.querySelector('.t-backend');
    if (!slide) return;
    var list = data().diagrams || [];
    if (!list.length) return;

    var panel = slide.querySelector('.cs-toggle-panel');
    var stage = slide.querySelector('.cs-diagram-stage');
    var caption = slide.querySelector('.cs-caption');
    ...
    CS.makeToggles(panel, list, function (d) {
      fill(shot, d.media);
      caption.textContent = d.caption || '';
    });
    fill(shot, list[0].media);
    caption.textContent = list[0].caption || '';
  };
